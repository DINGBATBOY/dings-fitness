...
import {
  ...
  Calendar, Star, Award, Dumbbell, Heart, Feather, ChevronRight, Play,
} from 'lucide-react';
import type {
...
export const Wrapped: React.FC<WrappedProps> = ({
  ...
  // Story mode — tap-through slides of the deep-dive sections.
  ...
  // ── Story slides — the deep-dive sections, one per tap-through card ──
  const storySlides = [
    { id: 'consistency', show: true, node: (
      <>
              <FadeIn>
    ...
    { id: 'foods', show: stats.topMeals.length > 0, node: (
      <>
    ...
    { id: 'body', show: (stats.weightDelta !== null || !!stats.firstWeight), node: (
      <>
    ...
    { id: 'closing', show: true, node: (
      <div className="text-center px-2 py-10">
        <Feather className="w-8 h-8 text-orange-300 mx-auto mb-4" strokeWidth={1.5} />
        <div className="text-[10px] uppercase tracking-[0.25em] text-orange-300 font-bold">Your currently forged path</div>
        <h3 className="text-3xl font-bold text-white mt-3">Walk on, {firstName}</h3>
        <p className="text-[13px] text-gray-400 mt-3">Your steps are forged. Take more.</p>
      </div>
    ) },
  ].filter(s => s.show);
      ...
      <div className={inline ? 'space-y-5 pb-2' : 'max-w-md mx-auto px-4 pb-32 pt-2 space-y-5'}>
          ...
          {stats.days > 0 && (
            <>
              ...
              <FadeIn>
                <button
                  onClick={() => { setStoryIdx(0); setStoryOpen(true); }}
                  className="w-full rounded-3xl border border-[#d97757]/40 bg-gradient-to-r from-[#d97757]/20 to-[#d4a55a]/10 px-5 py-4 flex items-center gap-3 text-left active:scale-[0.99] transition-transform"
                >
                  <div className="w-11 h-11 rounded-full bg-[#d97757] flex items-center justify-center shrink-0">
                    <Play className="w-5 h-5 text-white ml-0.5" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-bold text-white">Slide through your story</div>
                    <div className="text-[11px] text-gray-400 mt-0.5">
                      {storySlides.length} cards · consistency, fuel, training, and more
                    </div>
                  </div>
              ...
              {/* At a glance */}
              <FadeIn delay={0.05}>
                <SectionHeading icon={<Activity className="w-3.5 h-3.5" />} label="At a glance" />
                <div className="grid grid-cols-2 gap-2.5">
                  <GlanceCard label="Avg calories" value={stats.averages.calories.toLocaleString()} unit="kcal / day" />
                  <GlanceCard label="Avg protein" value={String(stats.averages.protein)} unit="g / day" />
                  <GlanceCard
                    label="Workouts"
                    value={String(period === 'week' ? stats.workoutsThisWeek : stats.totals.workouts)}
                    unit={period === 'week' ? 'this week' : 'this month'}
                  />
                  <GlanceCard
                    label="Weight"
                    value={stats.weightDelta === null ? '—' : (stats.weightDelta > 0 ? '+' : '') + stats.weightDelta.toFixed(1)}
                    unit={stats.weightDelta === null ? 'no weigh-ins' : 'lb change'}
                  />
                </div>
              </FadeIn>
        ...
        <div className="fixed inset-0 z-[240] bg-[#0d0a08] flex flex-col">
            ...
            {storySlides.map((s, i) => (
              <div key={s.id} className="flex-1 h-1 rounded-full bg-white/10 overflow-hidden">
                <div
                  ...
                  style={{ width: i <= storyIdx ? '100%' : '0%', opacity: i === storyIdx ? 1 : 0.5 }}
                />
              </div>
            ))}
          </div>
          <div className="flex items-center justify-between px-4 pt-3">
            <div className="text-[10px] uppercase tracking-widest font-bold text-orange-300">
              {firstName}'s {period === 'week' ? 'week' : 'month'} · {storyIdx + 1} of {storySlides.length}
            </div>
            <button
              onClick={() => setStoryOpen(false)}
              className="w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-gray-300 z-10"
          ...
          <div className="relative flex-1 flex items-center overflow-y-auto px-5 py-6">
            ...
            <button
              ...
              onClick={() => (storyIdx > 0 ? setStoryIdx(i => i - 1) : setStoryOpen(false))}
            />
            <button
              className="absolute inset-y-0 right-0 w-2/3"
              aria-label="Next card"
              onClick={() => (storyIdx < storySlides.length - 1 ? setStoryIdx(i => i + 1) : setStoryOpen(false))}
            />
          </div>
          <p
...
const GlanceCard: React.FC<{ label: string; value: string; unit: string }> = ({ label, value, unit }) => (
  <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
    <div className="text-[10px] uppercase tracking-widest text-gray-400 font-bold">{label}</div>
    <div className="text-2xl font-bold text-white tabular-nums leading-none mt-2">{value}</div>
