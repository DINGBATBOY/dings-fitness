...
/**
 * A single ingredient in a prepared meal. Populated for AI-analyzed meal
 * photos (chicken, rice, sauce, veg, etc.) so the user can drill into the
 * meal and see what makes it up. Kept optional on FoodItem because
 * single-item logs (an apple, a nutrition-label scan, a restaurant menu
 * item) don't have ingredients.
 */
...
export interface FoodItem {
  ...
  timestamp: string;
  /**
   * When present, this FoodItem was logged as a prepared meal that the
   * AI could decompose. The Log tab uses this to render a drill-in view
   * where the user can edit individual ingredient portions or delete an
   * ingredient. When absent, the item is treated as a single unit.
   */
...
export interface AppState {
  ...
  bodyStats: BodyStats;  lastActiveDate?: string;
